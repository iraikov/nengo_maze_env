...
with model:
    ...
    xy_reader_conn = nengo.Connection(place_reader.neurons, xy_reader, synapse=None,
    ...
    xy_node = nengo.Node(size_in=2)
    nengo.Connection(environment[:2], xy_node)
    
    nengo.Connection(xy_reader, xy_error[1:])
    nengo.Connection(xy_node, xy_error[1:], transform=-1)
...
if __name__ == '__main__':
    ...
    with model:
        ...
        p_xy_error = nengo.Probe(xy_error, synapse=tau_probe)
        p_xy_readout = nengo.Probe(xy_reader, synapse=tau_probe)
        p_xy = nengo.Probe(xy_node, synapse=tau_probe)
    ...
    with nengo.Simulator(model, progress_bar=True, dt=dt) as sim:
        ...
        plt.plot(sim.trange(), sim.data[p_xy_error],
                 alpha=0.8, label="XY error")
        plt.xlabel("Time (s)")
        plt.ylabel("Output")
        plt.legend()
        plt.show()
