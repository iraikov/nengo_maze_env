...
from reservoir_rmhl import NengoReservoirRMHL
...
#    u = nengo.Node(output=nengo.processes.PresentInput([0.1],  presentation_time=pulse_interval))
#    z = nengo.Node(output=nengo.processes.PresentInput([0.75],  presentation_time=pulse_interval))
    e = nengo.Node(size_in=1)
    
    learning = nengo.Node(size_in=1, output=lambda t,v: True if t <= T_train else False)

    rsvr = NengoReservoirRMHL(n_per_dim = n, dimensions=ndim, learning_rate=learning_rate,
...
with model:
    # Probes
    p_e = nengo.Probe(e, synapse=tau_probe)
...
plt.plot(sim.trange()[t_train], sim.data[p_e][t_train], alpha=0.8, label="Error signal")
